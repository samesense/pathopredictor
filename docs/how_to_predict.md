...
```
docker run -e USER=$USER -e USERID=$UI --user $(id -u) -it -v /local/data/path/:/opt/pathopredictor/data/
```

Put your vcf file(s) in `/local/data/path/interim/user_preds/`. Using the `interim/user_preds/` directory is required.
