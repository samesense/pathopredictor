...
def mk_var(row, genome):
    if '-' == row['ref']:
        ...
        pos = row['pos']
        ref = genome['chr' + row['chrom']][pos-1:pos]
        alt = get_alt(row)
        ls  = [row['chrom'], str(row['pos']), '.',
               ref, alt]
    return ls
...
def mk_vcf_line(row, genome, fout):
    ls = (row['clinical_class'], row['pos_fam_count'], row['neg_fam_count'], row['hom_fam_count'])
    info = 'CLIN_CLASS=%s;POS_FAM_COUNT=%d;NEG_FAM_COUNT=%d;POS_HOM_FAM_COUNT=%d' % ls
