...
def eval_sig(row, var_type):
    if row[var_type + '_qval'] < .01 and row[var_type + '_fg_gtr'] and row['pfam'] != 'none':
        return 'fg_sig'
    elif row[var_type + '_qval'] < .01 and not row[var_type + '_fg_gtr'] and row['pfam'] != 'none':
        return 'exac_sig'
    elif row[var_type + '_qval'] > .2 and row['pfam'] != 'none':
        return 'not_sig'
    else:
        return 'ignore'
...
def main(args):
    ...
    df.loc[:, 'sig'] = df.apply(lambda row: eval_sig(row, args.var_type), axis=1)
    ...
    #     eval_mpc(args.dat_file, args.var_type, fout)
