...
def mk_panel_clinvar_data(disease_df, query_df):
    ...
    disease_flag.loc[:, 'is_query'] = disease_flag.apply(lambda row: row['_merge']=='both', axis=1)
...
def load_data(args):
    ...
    for disease in diseases:
        data[disease] = mk_panel_clinvar_data(disease_df[disease_df.Disease==disease], query_df)
    return data
