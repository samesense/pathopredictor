...
crit = df_pre.apply(lambda row: 'dup' in row['Alt'] and row['Gene Symbol'] in FOCUS_GENES, axis=1)
df = df_pre[crit]
df.loc[:, 'id'] = df.apply(lambda row: row['Transcript'] + ':' + row['Pos'] + 'dup', axis=1)
...
#    print(val)
    ...
    print(v, c, chrom, offset, ref, fix_alt(chrom, offset, ref, v) + ref)
