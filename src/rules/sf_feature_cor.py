...
rule mk_cor_data:
    input:  i = DATA + 'interim/panel.dat'
    ...
    run:
        ...
        df_ls.append(mk_cor_data(df[crit], col_names, 'Epilepsy (dominant genes)'))
        pd.concat(df_ls).to_csv(output.o, index=False, sep='\t')

rule plot_feature_cor:
    input:  DATA + 'interim/feature_cor/plot_data'
    output: DOCS + 'paper_plts/fig2b_featureCor.pdf'
    run:
        R("""
          require(ggplot2)
          ...
          theme_bw(base_size=18) + scale_x_discrete(expand = c(0, 0)) +
