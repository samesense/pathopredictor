...
rule parse_vcf:
   ...
   run:
       with open(input.i) as f, open(output.o, 'w') as fout, open(output.o2, 'w') as fout_split_pfam:
           print('chrom\tpos\tref\talt\tclin_class\tpfam\taf_1kg_all\teff\tgene\tmpc\tmtr\texac_af\texac_ac\texac_an\texac_cov_frac\tkaviar_af\tProtein_Change\tHugo_Symbol', file=fout)
           print('chrom\tpos\tref\talt\tclin_class\tpfam\taf_1kg_all\teff\tgene\tmpc\tmtr',
           ...
           for line in f:
               if not line[0] == '#':
                   ...
                   ls = (chrom, pos, ref, alt, clin, pfam, onekg, eff,
                         gene, mpc, mtr, exac_af, exac_ac, exac_an,
                         exac_cov_frac, kv_af, protein_change, gene)
                   print('\t'.join(ls), file=fout)

                   for p in pfam.split(','):
                       ls = (chrom, pos, ref, alt, clin, p, onekg, eff, gene, mpc, mtr)
